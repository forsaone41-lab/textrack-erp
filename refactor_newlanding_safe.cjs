...
try {
  ...
  // 1. Add necessary imports if missing
...
`;
  ...
  const oldSubmitRegex = /const handleSubmit = async \(\) => \{[\s\S]*?setIsSubmitting\(false\);\n    \}\n  \};/;
  ...
  `;
  content = content.replace(oldSubmitRegex, newHandleSubmit);

  // 5. Replace photo upload with Calculator UI exactly by searching lines
  const lines = content.split('\\n');
  let startIdx = -1;
  let endIdx = -1;
  
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes("{formData.intent === 'production' && (") && lines[i+1].includes('animate-in fade-in duration-300')) {
      startIdx = i;
      break;
    }
  }
...
`;
  ...
  if (startIdx !== -1 && endIdx !== -1) {
    lines.splice(startIdx, endIdx - startIdx + 1, calculatorUI);
    content = lines.join('\\n');
  } else {
  ...
  console.log("Successfully rebuilt NewLanding.tsx!");
} catch(err) {
