...
// The exact string to replace in Clement
const oldWrapper = `{page === 'product' && (
...
if (c.includes(oldWrapper)) {
  ...
  const nextLayoutIndex = c.indexOf('// Contact'); // safe boundary
  ...
  console.log('Clement wrapper not found. Here is what exists:');
  const match = c.match(/\{page === 'product' && \([\s\S]{0,300}/);
  if(match) console.log(match[0]);
}

// Fix Mazia Layout too
const maziaBgOld = `<div className="flex-1 w-full bg-white relative">`;
const maziaBgNew = `<div className="flex-1 w-full relative" style={{ backgroundColor: cardBg }}>`;
if (c.includes(maziaBgOld)) {
  c = c.replace(maziaBgOld, maziaBgNew);
  fs.writeFileSync('src/pages/StoreBuilder.tsx', c, 'utf-8');
  console.log('Fixed Mazia bg');
}
