const fs = require('fs');
let content = fs.readFileSync('src/pages/AdsLanding.tsx', 'utf8');
...
// 4. Modify isSuccess block
const oldIsSuccessBlock = `<div className="space-y-4">
...
const newIsSuccessBlock = \`
          {newClientCode && (
            <div className="bg-white border-2 border-slate-100 rounded-[2rem] p-6 md:p-8 mb-8 text-right shadow-sm">
              <h3 className="text-xl font-black text-slate-800 mb-6 flex items-center justify-between">
                <span>معلومات حسابك الخاص</span>
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 p-4 rounded-xl flex items-center justify-between border border-slate-100">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">البريد الإلكتروني (Email)</p>
                ...
                <div className="bg-emerald-50 p-4 rounded-xl flex items-center justify-between border border-emerald-100">
                  <div>
                    <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">الرمز السري (Code)</p>
          ...
          <div className="space-y-4">
            <a 
              href={\\\`https://wa.me/\\\${company.phone ? company.phone.replace(/\\\\D/g, '') : '212624465962'}?text=\\\${encodeURIComponent(\\\`مرحباً BEYA CREATIVE، لقد سجلت طلبي للتو باسم \\\${submittedName}. \\\${newClientCode ? \\\`الرمز السري الخاص بي هو: \\\${newClientCode.code}.\\\` : ''} أريد تأكيد الطلب والبدء في العينة. \\\${totalEstimate ? \\\`(التكلفة التقديرية التي ظهرت لي: \\\${totalEstimate.min} - \\\${totalEstimate.max} درهم)\\\` : ''}\\\`)}\\\`}
              target="_blank"
              rel="noreferrer"
              className="w-full py-5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl font-black text-lg transition-all shadow-xl shadow-emerald-200 flex items-center justify-center gap-3"
            >
...
content = content.replace(oldIsSuccessBlock, newIsSuccessBlock);
...
fs.writeFileSync('src/pages/AdsLanding.tsx', content);
console.log('done');
