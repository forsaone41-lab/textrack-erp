    ...
    <div className={\`min-h-screen bg-slate-900 text-white relative overflow-hidden \${isAr ? 'font-sans' : ''}\`} dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-[100] bg-slate-900/50 backdrop-blur-xl border-b border-white/5 transition-all">
        <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to={isLoggedIn ? "/dashboard" : "/login"} className="px-6 md:px-8 py-3 bg-white text-slate-900 rounded-xl text-xs font-black uppercase tracking-widest hover:scale-105 transition-transform">
      ...
      <div className="relative w-full h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-5xl mx-auto mt-20">
          <div className="inline-flex items-center gap-2 px-6 py-2 bg-white/10 backdrop-blur-md rounded-full text-[10px] md:text-xs font-black text-white uppercase tracking-[0.2em] mb-8 border border-white/20 animate-fade-in-up">
            <Star className="w-4 h-4 text-amber-400 fill-amber-400" />
            {isAr ? 'الشريك الأول لنجاحك الرقمي في المغرب' : 'Le 1er Partenaire de votre réussite digitale au Maroc'}
          </div>
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-black mb-6 tracking-tighter leading-[1.1] uppercase text-white animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          ...
          <p className="text-lg md:text-2xl text-slate-300 mb-12 font-medium max-w-3xl mx-auto leading-relaxed animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            {isAr 
              ? 'سواء كنت تملك متجراً وتحتاج إلى تصنيع ملابس عالية الجودة، أو كنت تبدأ من الصفر وتحتاج لمتجر احترافي قوي كـ Shopify، نحن هنا لنحقق ذلك بأعلى معدل تحويل (Conversion Rate).' 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div className="bg-slate-800/80 backdrop-blur-xl border border-blue-500/30 rounded-[3rem] p-10 lg:p-14 hover:bg-slate-800 transition-all group relative overflow-hidden flex flex-col justify-between">
              ...
              <button 
                ...
                className="inline-flex items-center justify-center gap-3 w-full px-8 py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase tracking-widest transition-colors shadow-lg shadow-blue-900/50"
              >
            ...
            <div className="bg-slate-800/80 backdrop-blur-xl border border-purple-500/30 rounded-[3rem] p-10 lg:p-14 hover:bg-slate-800 transition-all group relative overflow-hidden flex flex-col justify-between">
              ...
              <div>
                ...
                <p className="text-slate-400 text-lg mb-10 leading-relaxed font-medium">
                  {isAr 
                    ? 'لديك موقع مسبقاً وتحتاج فقط إلى سلع ذات جودة عالية؟ نحن نقدم لك تصنيعاً مخصصاً (تيشيرت، هودي، إلخ) بدقة متناهية، مع التغليف والخدمات اللوجستية المتكاملة.' 
                    : 'Vous avez déjà votre site et cherchez des produits premium ? Nous fabriquons vos vêtements sur-mesure (T-shirts, Hoodies, etc.) avec une précision extrême et un packaging soigné.'}
                </p>
              </div>
              <button onClick={() => document.getElementById('contact-form')?.scrollIntoView({behavior:'smooth'})} className="inline-flex items-center justify-center gap-3 w-full px-8 py-5 bg-purple-600 hover:bg-purple-500 text-white rounded-2xl font-black uppercase tracking-widest transition-colors shadow-lg shadow-purple-900/50">
      ...
      <section id="contact-form" className="py-32 px-6 bg-white relative">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <form onSubmit={async (e) => {
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-[2rem] p-6 md:p-10 shadow-xl">
                <div className="flex items-center justify-center gap-4 mb-10">
                  <div className={\`h-2 rounded-full transition-all \${simulatorStep >= 1 ? 'w-20 bg-purple-600' : 'w-6 bg-slate-200'}\`}></div>
                  <div className={\`h-2 rounded-full transition-all \${simulatorStep >= 2 ? 'w-20 bg-purple-600' : 'w-6 bg-slate-200'}\`}></div>
                </div>

                {simulatorStep === 1 && (
                  <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                    {models.map((m, idx) => (
                      <div key={m.id} className="border border-slate-200 bg-white rounded-2xl p-6 space-y-6">
                        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                          <h3 className="font-black text-slate-800 text-lg uppercase tracking-widest text-purple-600">{isAr ? 'الموديل' : 'Modèle'} {idx + 1}</h3>
                          {models.length > 1 && (
                            <button type="button" onClick={() => setModels(prev => prev.filter(x => x.id !== m.id))} className="text-rose-500 hover:bg-rose-50 p-2 rounded-lg transition-colors">
                              <X className="w-5 h-5" />
                            </button>
                          )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">{isAr ? 'نوع اللباس' : 'Type de vêtement'}</label>
                            <div className="relative">
                              <select value={m.type} onChange={e => updateModel(m.id, { type: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl py-3 px-4 text-sm font-bold outline-none focus:border-purple-600 transition-colors appearance-none text-slate-900">
                            ...
                            {m.type === 'Autre' && (
                              <input type="text" value={m.customType} onChange={e => updateModel(m.id, { customType: e.target.value })} placeholder={isAr ? 'حدد النوع' : 'Spécifiez'} className="mt-3 w-full bg-slate-50 border-2 border-slate-200 rounded-xl py-3 px-4 text-sm font-bold outline-none focus:border-purple-600 text-slate-900" required />
                            )}
                          </div>
                          <div>
                            <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">{isAr ? 'الكمية الإجمالية' : 'Quantité Totale'}</label>
                            <input type="number" min="1" placeholder="100" value={m.quantity} onChange={e => updateModel(m.id, { quantity: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl py-3 px-4 text-sm font-bold outline-none focus:border-purple-600 text-slate-900" required />
                          </div>
                        </div>

                        <div>
                          <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-3">{isAr ? 'تفاصيل (ألوان، طباعة...)' : 'Détails (Couleurs, Impression...)'}</label>
                          <textarea rows={3} value={m.details} onChange={e => updateModel(m.id, { details: e.target.value })} className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl py-3 px-4 text-sm font-bold outline-none focus:border-purple-600 resize-none text-slate-900" />
                        </div>

                        <div>
                          <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-3">{isAr ? 'صورة الموديل (إجباري)' : 'Photo du modèle (Obligatoire)'}</label>
                          <div className="flex flex-wrap gap-3">
                            {(m.photos || (m.photo ? [m.photo] : [])).map((p, pIdx) => (
                              <div key={pIdx} className="relative w-20 h-20 rounded-xl overflow-hidden border-2 border-slate-200">
                            ...
                            <label className="w-20 h-20 bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-purple-500 hover:bg-purple-50 transition-colors">
                              <ImageIcon className="w-5 h-5 text-slate-400 mb-1" />
                              <span className="text-[9px] font-black uppercase text-slate-500">Ajouter</span>
                        ...
                        {(() => {
                          ...
                          if (!est) return null;
                          return (
                            <div className="p-4 bg-purple-50 border-2 border-purple-100 rounded-xl flex items-center justify-between mt-4">
                              <span className="text-xs font-black text-purple-600 uppercase">{isAr ? 'التكلفة التقديرية:' : 'Coût Estimé:'}</span>
                              <span className="text-sm font-black text-slate-900">{est.totalMin.toLocaleString()} - {est.totalMax.toLocaleString()} MAD</span>
                            </div>
                          );
                        })()}
                      </div>
                    ...
                    <button type="button" onClick={() => setModels(prev => [...prev, emptyModel()])} className="w-full py-4 border-2 border-dashed border-slate-300 text-slate-600 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-100 transition-colors">
                      + {isAr ? 'إضافة موديل' : 'Ajouter un modèle'}
                    </button>

                    <button type="submit" className="w-full py-5 bg-purple-600 text-white rounded-2xl font-black text-lg uppercase tracking-widest shadow-xl shadow-purple-600/30 hover:bg-purple-700 transition-colors flex justify-center items-center gap-2">
                ...
                {simulatorStep === 2 && (
                  <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
                    <h3 className="text-2xl font-black uppercase tracking-tight text-slate-900 mb-6">{isAr ? 'معلومات التواصل' : 'Vos coordonnées'}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">{isAr ? 'الإسم الكامل' : 'Nom Complet'}</label>
                        <input type="text" name="name" className="w-full bg-white border-2 border-slate-200 rounded-xl py-3 px-4 font-bold text-slate-900 outline-none focus:border-purple-600" required />
                      </div>
                      <div>
                        <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">Email</label>
                        <input type="email" name="email" className="w-full bg-white border-2 border-slate-200 rounded-xl py-3 px-4 font-bold text-slate-900 outline-none focus:border-purple-600" required />
                      </div>
                      <div>
                        <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">{isAr ? 'الهاتف / الواتساب' : 'Téléphone'}</label>
                        <div className="flex gap-2">
                          <select name="countryCode" className="w-24 bg-white border-2 border-slate-200 rounded-xl px-2 font-bold outline-none focus:border-purple-600 appearance-none text-center">
                            <option value="+212">+212</option>
                            <option value="+33">+33</option>
                          </select>
                          <input type="tel" name="phone" className="flex-1 bg-white border-2 border-slate-200 rounded-xl py-3 px-4 font-bold text-slate-900 outline-none focus:border-purple-600" required dir="ltr"/>
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs font-black text-slate-700 uppercase tracking-widest mb-2">{isAr ? 'المدينة' : 'Ville'}</label>
                        <input type="text" name="ville" className="w-full bg-white border-2 border-slate-200 rounded-xl py-3 px-4 font-bold text-slate-900 outline-none focus:border-purple-600" required />
                      </div>
                    </div>

                    <div className="flex gap-4 pt-4">
                      <button type="button" onClick={() => setSimulatorStep(1)} className="px-6 py-5 bg-slate-100 text-slate-600 rounded-2xl font-black uppercase tracking-widest hover:bg-slate-200 transition-all">{isAr ? 'رجوع' : 'Retour'}</button>
                      <button type="submit" className="flex-1 py-5 bg-emerald-500 text-white rounded-2xl font-black text-lg uppercase tracking-widest shadow-xl shadow-emerald-500/30 hover:bg-emerald-600 transition-all">
      ...
      {/* Footer */}
      <footer className="py-12 bg-slate-900 text-slate-400 text-center text-xs font-black uppercase tracking-widest border-t border-white/5">
