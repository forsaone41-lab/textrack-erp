...
content = content.replace(/{ id: 'fitness-pulse', name: 'Fitness Pulse', layout: 'hero-center'/g, "{ id: 'fitness-pulse', name: 'Joyride (Pro)', layout: 'pro-joyride'");

const layoutRouterMatch = `if (activeTheme.layout === 'pro-lamode') return <LayoutProLamode {...props} />;`;
const newLayoutRouter = `if (activeTheme.layout === 'pro-joyride') return <LayoutProJoyride {...props} />;\n       ` + layoutRouterMatch;
content = content.replace(layoutRouterMatch, newLayoutRouter);

fs.writeFileSync(filePath, content);
console.log('LayoutProJoyride injected successfully.');
