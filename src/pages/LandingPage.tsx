...
export default function LandingPage() {
  ...
  // Track when the hero video enters the viewport
  useEffect(() => {
            ...
            <div className="relative shrink-0">
              ...
              {showLangMenu && (
                <>
                  ...
                  <div className="absolute top-full right-0 mt-2 w-24 bg-white border border-slate-100 rounded-xl shadow-xl animate-fade-in flex flex-col overflow-hidden z-[60]">
      ...
      <section className="pt-28 pb-8 md:pb-32 px-6 flex items-center relative overflow-hidden bg-white min-h-[100dvh]">
        <div className="max-w-7xl mx-auto relative z-10 grid lg:grid-cols-2 gap-12 items-center w-full">
          <div className={isAr ? 'text-center lg:text-right' : 'text-center lg:text-left'}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 animate-in slide-in-from-bottom-8 duration-1000 delay-500">
              <button onClick={() => setIsModalOpen(true)} className="w-full sm:w-auto px-6 py-2.5 rounded-full bg-[#0071e3] hover:bg-[#0077ED] text-white font-semibold text-base transition-all flex items-center justify-center gap-2 shadow-lg hover:shadow-xl hover:-translate-y-0.5 cursor-pointer">
                {t.heroStartProject} <ArrowRight className={`w-4 h-4 ${isAr ? 'rotate-180' : ''}`} />
              </button>
              <Link to="/ecosystem" className="w-full sm:w-auto px-6 py-2.5 rounded-full bg-white border border-slate-200 text-slate-700 font-medium text-base hover:bg-slate-50 transition-all flex items-center justify-center gap-2 shadow-sm">
                {t.heroDetails} <ChevronRight className={`w-3.5 h-3.5 ${isAr ? 'rotate-180' : ''}`} />
              </Link>
              <a
                href="https://wa.me/212675239885"
                target="_blank"
                rel="noopener noreferrer"
                className="hidden sm:flex items-center gap-1.5 text-slate-800 hover:text-black font-semibold text-base cursor-pointer transition-colors ml-2"
                title={isAr ? 'استشارة مجانية عبر الواتساب' : 'Consultation gratuite WhatsApp'}
              >
                <svg viewBox="0 0 24 24" width="20" height="20" fill="#25D366" className="shrink-0"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.888-.788-1.487-1.761-1.66-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/></svg>
                <span>{isAr ? 'استشارة' : 'Consultation'}</span>
              </a>
            </div>
          </div>

          <div id="hero-mobile-container" className="w-full flex flex-col items-center justify-center min-h-[100dvh] lg:min-h-0 lg:sticky lg:bottom-8">
            <div id="hero-video" ref={heroFocus.ref} style={heroFocus.style} className="relative rounded-2xl md:rounded-[2rem] overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.12)] border-b-2 border-[#0071e3] w-full h-[calc(100dvh-150px)] lg:h-[calc(100vh-130px)] lg:max-h-[820px] md:aspect-[4/5] md:w-auto mx-auto animate-in slide-in-from-bottom-8 duration-1000 delay-150 group">
              <video
                ref={heroVideoRef}
                autoPlay
                loop
            ...
            <div className="lg:hidden flex flex-col items-center w-full mt-4 gap-3 px-1">
              ...
              >
                <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.888-.788-1.487-1.761-1.66-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/></svg> {isAr ? 'استشارة مجانية عبر الواتساب' : 'Consultation gratuite WhatsApp'}
              </a>
              <button
