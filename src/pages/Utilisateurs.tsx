...
import {
  ...
  Globe, Trophy, UserCircle, RotateCcw, Lock, Zap, MousePointer2, UserCheck, BarChart3, Settings, X, Sparkles, Inbox as InboxIcon, Mail, Video, Briefcase
} from 'lucide-react';
import {
...
const ROLE_CFG = {
  ...
  commercial: {
    label: 'Commercial', labelAr: 'تجاري (Commercial)', icon: Briefcase,
    bg: 'bg-sky-50', text: 'text-sky-700', border: 'border-sky-200',
    ring: 'ring-sky-400', avatar: 'from-sky-500 to-blue-600', dot: 'bg-sky-500',
    desc: 'Portail commercial, leads et CRM',
    access: ['Portail Commercial', 'CRM', 'Devis'],
  },
} as const;
...
const ALL_PAGES: PageDef[] = [
  ...
  { key: 'tarifs_edit', label: 'Édition Tarifs', labelAr: 'تعديل الأسعار', icon: ShoppingBag, group: 'Général' },
...
const LOCKED: Partial<Record<'admin' | 'pointeur' | 'client' | 'worker' | 'coupeur' | 'modeliste' | 'controleur' | 'agent_pointage' | 'partenaire' | 'chef_chaine' | 'commercial', AppPage[]>> = {
  admin: ['utilisateurs'],
  client: ['portail_client'],
  chef_chaine: ['chef_chaine_portal'],
  commercial: ['commercial_portal'],
};
...
export default function Utilisateurs() {
  ...
  // ── Permissions helpers ───────────────────────────────────
  function togglePerm(role: 'admin' | 'pointeur' | 'chef_chaine' | 'client' | 'worker' | 'coupeur' | 'modeliste' | 'controleur' | 'agent_pointage' | 'partenaire' | 'commercial', page: AppPage) {
  ...
  function resetRole(role: 'admin' | 'pointeur' | 'client' | 'worker' | 'coupeur' | 'modeliste' | 'controleur' | 'agent_pointage' | 'partenaire' | 'chef_chaine' | 'commercial') {
      ...
      {tab === 'users' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {(['admin', 'pointeur', 'chef_chaine', 'client', 'worker', 'coupeur', 'modeliste', 'controleur', 'agent_pointage', 'partenaire', 'commercial'] as const).map(role => {
                ...
                {PAGE_GROUPS.map(group => (
                  <Fragment key={group}>
                    <tr className="bg-slate-50/30">
                      <td colSpan={12} className={`px-6 py-2 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] ${isAr ? 'text-right' : 'text-left'}`}>
            ...
            <div className="p-10 space-y-6 max-h-[70vh] overflow-y-auto scrollbar-hide">
              ...
              <div className={isAr ? 'text-right' : ''}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(['admin', 'pointeur', 'chef_chaine', 'client', 'worker', 'coupeur', 'modeliste', 'controleur', 'agent_pointage', 'partenaire', 'commercial'] as const).map(r => (
